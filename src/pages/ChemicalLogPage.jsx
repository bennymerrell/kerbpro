...
function LogForm({ onSave, onCancel }) {
  ...
  async function handleSubmit(e) {
    ...
    await base44.entities.ChemicalLog.create({
      ...
      end_amount: parseFloat(form.end_amount),
    });
    setSaving(false);
    onSave();
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-card border border-border rounded-xl p-4 space-y-3 mb-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div>
          <label className="text-xs text-muted-foreground font-medium">End Amount</label>
          <input type="number" step="0.01" value={form.end_amount} onChange={e => set('end_amount', e.target.value)} placeholder="0.00" required
            className="w-full mt-1 h-9 px-2 rounded-lg border border-input bg-background text-xs text-foreground focus:outline-none focus:ring-2 focus:ring-primary/30" />
        </div>
        <div>
...
export default function ChemicalLogPage() {
  ...
  const [view, setView] = useState('personal'); // 'personal' | 'company'
  ...
  function calcUsage(log) { return (log.start_amount - log.end_amount).toFixed(2); }

  function totalUsage(logList) {
    return logList.reduce((acc, l) => acc + (l.start_amount - l.end_amount), 0).toFixed(2);
  }

  // Group by chemical for overview
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="bg-card border-b border-border px-4 py-3 flex items-center gap-3">
        ...
        <div className="ml-auto flex items-center gap-2">
          <div className="flex rounded-lg border border-border overflow-hidden">
            <button onClick={() => setView('personal')} className={`px-3 py-1.5 text-xs font-medium transition-colors ${view === 'personal' ? 'bg-primary text-primary-foreground' : 'text-foreground hover:bg-muted/60'}`}>
              My Usage
            </button>
            <button onClick={() => setView('company')} className={`px-3 py-1.5 text-xs font-medium transition-colors ${view === 'company' ? 'bg-primary text-primary-foreground' : 'text-foreground hover:bg-muted/60'}`}>
              Company
            </button>
          </div>
      ...
      <div className="max-w-3xl mx-auto px-4 py-6 space-y-6">
        {showForm && (
          <LogForm onSave={() => { setShowForm(false); loadLogs(); }} onCancel={() => setShowForm(false)} />
        )}
        ...
        <div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            ...
            {Object.entries(chemMap).slice(0, 1).map(([name, data]) => (
              <StatCard key={name} label={name} value={data.used.toFixed(2)} unit={data.unit} color="text-primary" />
            ))}
          </div>

          {/* Per-chemical breakdown */}
          {Object.keys(chemMap).length > 1 && (
                ...
                <div key={log.id} className="px-4 py-3 flex items-start gap-3">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      {view === 'company' && <span className="text-[10px] text-muted-foreground truncate">{log.created_by}</span>}
                    </div>
                    <div className="flex items-center gap-3 mt-1 text-xs text-muted-foreground">
                      <span>Start: <strong className="text-foreground">{log.start_amount} {log.unit}</strong></span>
                      <span>End: <strong className="text-foreground">{log.end_amount} {log.unit}</strong></span>
                      <span>Used: <strong className="text-destructive">{calcUsage(log)} {log.unit}</strong></span>
                    </div>
                    {log.notes && <div className="text-[10px] text-muted-foreground mt-0.5">{log.notes}</div>}
                  </div>
                  {log.created_by === currentUser?.email && (
                    <button onClick={() => handleDelete(log.id)} className="p-1.5 rounded-lg hover:bg-destructive/10 text-muted-foreground hover:text-destructive transition-colors">
